# Contributing to ep_ai_core

Thanks for helping improve `ep_ai_core` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_ai_core` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_ai_core from your local clone
pnpm run plugins i --path ../ep_ai_core

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_ai_core/` are picked up after a server restart.
...
* Etherpad version, ep_ai_core version, browser + OS
* Steps to reproduce
